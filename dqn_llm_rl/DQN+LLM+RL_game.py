...
def train_dqn(episodes=1000, batch_size=8):
...
def plot_training(rewards, agent_wins, enemy_wins, moves, success_rate):
    ...
    plt.savefig("Reward_LLM_RL_1000.png")
    plt.show()
    ...
    plt.savefig("Cumulative_Win_LLM_RL_1000.png")
    plt.show()
    ...
    plt.xlabel('Episodes')
    plt.ylabel('Moves')
    plt.savefig("Moves_LLM_RL_1000.png")
    plt.show()
    ...
    plt.legend()
    plt.savefig("Success_rate_LLM_RL_1000.png")
    plt.show()
...
def export_success_rate(success_rate):
    ...
    df.to_csv('success_rate_model_dqn_rl_1000.csv', index=False)
